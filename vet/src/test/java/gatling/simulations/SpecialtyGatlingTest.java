...
public class SpecialtyGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all specialties").get("/services/vet/api/specialties").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new specialty")
                        .post("/services/vet/api/specialties")
                ...
                .repeat(5)
                .on(
                    exec(http("Get created specialty").get("/services/vet${new_specialty_url}").headers(headersHttpAuthenticated)).pause(10)
                )
                .exec(http("Delete created specialty").delete("/services/vet${new_specialty_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
