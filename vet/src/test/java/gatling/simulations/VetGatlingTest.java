...
public class VetGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all vets").get("/services/vet/api/vets").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new vet")
                        .post("/services/vet/api/vets")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created vet").get("/services/vet${new_vet_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created vet").delete("/services/vet${new_vet_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
