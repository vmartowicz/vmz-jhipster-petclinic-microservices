...
public class PetTypeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all petTypes").get("/services/customer/api/pet-types").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new petType")
                        .post("/services/customer/api/pet-types")
                ...
                .repeat(5)
                .on(
                    exec(http("Get created petType").get("/services/customer${new_petType_url}").headers(headersHttpAuthenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created petType").delete("/services/customer${new_petType_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
