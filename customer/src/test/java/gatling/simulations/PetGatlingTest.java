...
public class PetGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all pets").get("/services/customer/api/pets").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new pet")
                        .post("/services/customer/api/pets")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created pet").get("/services/customer${new_pet_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created pet").delete("/services/customer${new_pet_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
